...
var collectFieldSourcesInTrace = function (plotEl, trace, traceIndex) { return walkObject(trace, function (key, d, currentPathArray) {
  ...
  var crossfilter = plotEl._crossfilter;
...
var persistCrossfilter = function (plotEl, crossfilter) {
  plotEl._crossfilter = crossfilter;
};

var restylePlots = function (Plotly, crossfilter, plotArray) {

  var someConstrained = crossfilter.plotDimensions.concat(crossfilter.formDimensions).some(function (dim) { return dim.constrained; });
  var newSet = someConstrained ? crossfilter.recordDimension.top(Infinity).sort(function (a, b) { return a.index - b.index; }) : [];
  ...
  plotArray.filter(function (gd) { return gd.includedInCrossfilter; }).forEach(function (ogd) {
    ...
    var plotFields = [].concat.apply([], values(crossfilter.fieldSources))
...
var resetAllFilters =
        function (crossfilter) { return crossfilter.plotDimensions.concat(crossfilter.formDimensions).forEach(function (dim) {
...
var resetCrossfilter = function (Plotly) {
  ...
  var crossfilter = plots[0]._crossfilter;
  resetAllFilters(crossfilter);
  restylePlots(Plotly, crossfilter, [].concat( plots ).filter(function (gd) { return gd.includedInCrossfilter; }));
};
...
var specFilter = function (Plotly, spec) {
  ...
  var crossfilter = plots[0]._crossfilter;
  var dim = crossfilter.formDimensions[0];
  ...
  dim.cfDimension = crossfilter.cf.dimension(function (d) { return d[gridColumn]; });
...
var selectionHandler = function (Plotly, gd, inputEventData) {
  ...
  var crossfilter = gd._crossfilter;
  var plotDimension = crossfilter.plotDimensions.find(function (dim) { return dim.gd === gd; });
  ...
  restylePlots(Plotly, crossfilter, getPlotArray(document).filter(function (gd) { return gd.includedInCrossfilter; }));
};

var cfPlotSetup = function (Plotly, plotArray, gdPromise, plotEl, plotContent) {

  var crossfilter = getCrossfilter(plotArray);
  ...
  Object.keys(crossfilter.fieldSources).forEach(function (gridColumnName) {
    var fieldSource = crossfilter.fieldSources[gridColumnName];
    ...
    crossfilter.plotDimensions = getPlotArray(document)
      ...
      .map(function (gd) {
        return {
          gd: gd,
          constrained: false,
          cfDimension: crossfilter.cf.dimension(function (d) { return d.index; })
        };
      });
  ...
  crossfilter.recordDimension = crossfilter.cf.dimension(function (d) { return d; });

  var gridArray = toPairs(crossfilter.grid);
